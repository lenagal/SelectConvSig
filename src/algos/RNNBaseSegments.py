...
class SelectConvSigRNNBase:
    ...
    def build_model(self,input_shape,signature_deg,classes):
        dimV=input_shape[1]
        logsiglen = iisignature.logsiglength(input_shape[1]*input_shape[2], signature_deg)
        ...
        mid_output = Lambda(lambda x: SP(x, self.N_SEGMENTS), output_shape=(self.N_SEGMENTS, self.FILTER_SIZE_2), name='start_position')(reshape_layer)
        # calculating segments log signatures
        hidden_layer = Lambda(lambda x: CLF(x, self.N_SEGMENTS, signature_deg, logsiglen), output_shape=(self.N_SEGMENTS, logsiglen), name='logsig')(reshape_layer)
        ...
        # inception layer
        joint_config_layer_k3=Conv1D(self.FILTER_SIZE_2,kernel_size=1,dim=dimV,activation='relu',padding='same',use_bias=False,name="temporal_config_layer1")(reduction_layer1)
        # #joint_config_layer=MaxPooling2D(pool_size=(2,1),strides=(2,1),padding='same',name="pooling_layer1")(joint_config_layer)
        # joint_config_layer_k5=LinConv(self.FILTER_SIZE_2,kernel_size=5,dim=dimV,activation='relu', padding='same',use_bias=False,name="config_layer2")(reduction_layer1)
        # concat_layer1 = concatenate([joint_config_layer_k1,joint_config_layer_k3,joint_config_layer_k5], axis=-1)
